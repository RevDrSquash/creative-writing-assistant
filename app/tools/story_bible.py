...
from app.tools.validation import reject_leaked_tool_markup_fields
...
_NARRATIVE_STYLE_PROSE = (
    "Plain prose for this field only. Pass as its own JSON argument; omit to leave unchanged."
)


@tool
def update_narrative_style(
    premise: Annotated[str | None, Field(description=_NARRATIVE_STYLE_PROSE)] = None,
    tone: Annotated[str | None, Field(description=_NARRATIVE_STYLE_PROSE)] = None,
    themes: Annotated[str | None, Field(description=_NARRATIVE_STYLE_PROSE)] = None,
    writing_style: Annotated[str | None, Field(description=_NARRATIVE_STYLE_PROSE)] = None,
) -> str:
    """Update narrative style fields (premise, tone, themes, writing_style).

    Each of those names is a separate JSON argument containing only that field's
    plain prose. Omit a field to leave it unchanged. Do not pack later fields
    into an earlier argument.
    """
    ...
    reject_leaked_tool_markup_fields(updates)
...
_CHARACTER_PROSE = "Plain prose for this identity field only. Pass as its own JSON argument; omit to leave unchanged."


@tool
def upsert_character(
    character_id: str = "",
    name: Annotated[str | None, Field(description=_CHARACTER_PROSE)] = None,
    traits: Annotated[str | None, Field(description=_CHARACTER_PROSE)] = None,
    appearance: Annotated[str | None, Field(description=_CHARACTER_PROSE)] = None,
    background: Annotated[str | None, Field(description=_CHARACTER_PROSE)] = None,
    voice: Annotated[str | None, Field(description=_CHARACTER_PROSE)] = None,
    intimacies: list[Intimacy] | None = None,
) -> str:
    """Create a character, or partially update one when `character_id` is given.

    Only the provided fields change. `name`, `traits`, `appearance`,
    `background`, and `voice` are separate JSON arguments, each containing only
    that field's plain prose. `intimacies` is its own JSON array of objects.
    It sets the character's BASELINE state (start of timeline); mid-story
    changes belong on events as signals. `intimacies` replaces the whole
    baseline intimacy list, so read the character first and resend the full
    list when editing it.
    ...
    reject_leaked_tool_markup_fields(identity_updates)
