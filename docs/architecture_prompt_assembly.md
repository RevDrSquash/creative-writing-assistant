...
* Prefix injection runs in the public `generate` / `agenerate` / `stream` / `astream` entry points (before `on_chat_model_start`), so the merged prompt is what both the model API and the local debug log see. If the request already has a leading `SystemMessage` (for example from `create_agent(system_prompt=...)` or a structured-output workflow node that passes `SystemMessage(primer + node instructions)`), the prefix is merged into it; otherwise a new leading `SystemMessage` is inserted. `_apply_prefix` is idempotent so the `stream` -> `invoke` fallback cannot double-apply the prefix.
