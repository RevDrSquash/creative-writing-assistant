...
* Propose-scene tool (replaces `draft_scene`): Change the main agent's `draft_scene` tool into
  a `propose_scene` tool that creates a *proposed scene* (a "scene card") instead of launching
